...
export default function App() {
  ...
  const handleCopy = (item: AlbumItem) => {
  ...
  const openYTMusic = (e: React.MouseEvent, item: AlbumItem) => {
    e.stopPropagation();
    handleCopy(item);
  ...
  const openRYM = (e: React.MouseEvent, item: AlbumItem) => {
    e.stopPropagation();
    handleCopy(item);
                  ...
                  <ul className="space-y-2">
                    {filteredPending.length === 0 ? (
                      ...
                      filteredPending.map(item => (
                        ...
                        >
                          <div className="break-words pr-4 flex flex-wrap gap-1 items-center flex-1">
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0">
                            ...
                            <div className={`transition-colors p-1 ${copiedFeedbackId === item.id ? 'text-green-500' : 'text-gray-300 group-hover:text-indigo-500'}`}>
                      ...
                      filteredCompleted.map(item => (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0">
                            <button 
                              onClick={(e) => openRYM(e, item)}
                              className="px-2 py-1 text-xs font-medium bg-purple-100 text-purple-700 rounded hover:bg-purple-200 transition-colors opacity-70 hover:opacity-100"
                              title="Abrir en RateYourMusic"
                            >
                              RYM
                            </button>
                            <button 
                              onClick={(e) => openYTMusic(e, item)}
                              className="px-2 py-1 text-xs font-medium bg-red-100 text-red-700 rounded hover:bg-red-200 transition-colors opacity-70 hover:opacity-100"
                              title="Abrir en YouTube Music"
                            >
                              YT Music
                            </button>
